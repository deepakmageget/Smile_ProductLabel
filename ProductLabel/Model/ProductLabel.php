...
class ProductLabel extends AbstractModel implements IdentityInterface, ProductLabelInterface
{
    ...
    {
        $values = $this->getData(self::PRODUCTLABEL_CATEGORY_ID);
        if (is_numeric($values)) {
            $values = [$values];
        }
    ...
    public function setCategoryId(array $value): ProductLabelInterface
    {
        return $this->setData(self::PRODUCTLABEL_CATEGORY_ID, $value);
    }
    /**
    ...
    public function populateFromArray(array $values): void
    {
        ...
        $this->setData(self::PRODUCTLABEL_DISPLAY_ON, $values['display_on']);
        if(isset($values['category_id'])){
            $this->setData(self::PRODUCTLABEL_CATEGORY_ID, implode(',', $values['category_id']));
        }
